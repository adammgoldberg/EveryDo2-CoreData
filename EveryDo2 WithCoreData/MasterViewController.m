...
//    ToDo *firstItem = [[ToDo alloc] initWithTitle:@"Walk the Dog" description:@"When walking the dog, make sure to walk for at least 3km and to stop at the park" priority:1 inMOC:self.managedObjectContext];
//    
//    ToDo *secondItem = [[ToDo alloc] initWithTitle:@"Go shopping" description:@"Don't forget to buy milk, eggs, and fruit" priority:3 inMOC:self.managedObjectContext];
//    
//    ToDo *thirdItem = [[ToDo alloc] initWithTitle:@"Clean the house" description:@"People are coming over on the weekend. The house needs to be clean." priority:4 inMOC:self.managedObjectContext];
//    
//    ToDo *fourthItem = [[ToDo alloc] initWithTitle:@"Return clothes" description:@"The shirt doesn't fit well. Go to the store to return it." priority:5 inMOC:self.managedObjectContext];
//    
//    ToDo *fifthItem = [[ToDo alloc] initWithTitle:@"Fix the sink" description:@"The sink is leaky and needs to be fixed. Go get tools from the neighbour" priority:2 inMOC:self.managedObjectContext];
//
...
- (void)insertNewObject:(id)sender {
    ...
    [newManagedObject setValue:[NSDate date] forKey:@"blah"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"showDetail"]) {
        ...
        [controller setDetailItem:aToDo];
        controller.navigationItem.leftBarButtonItem = self.splitViewController.displayModeButtonItem;
        controller.navigationItem.leftItemsSupplementBackButton = YES;
    }
    else if ([[segue identifier] isEqualToString:@"showModal"]) {
        ModalViewController *mdlcontroller = (ModalViewController *)[segue destinationViewController];
        mdlcontroller.managedObjectContext = self.managedObjectContext;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ToDoCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell" forIndexPath:indexPath];
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"itemTitle" ascending:NO];
